---
title: AI Team Roster
updated: YYYY-MM-DD
---

# AI Team Roster

## Leadership

| Name | Role | Specialty | Model | Status |
|------|------|-----------|-------|--------|
| **[Owner]** | Team Lead | Direction, decisions, final say | Human | Active |
| **Hal** | Orchestrator | Task routing, coordination, delegation | opus | Active |

## Core Team

| Name | Role | Specialty | Model | Status |
|------|------|-----------|-------|--------|
| **Holly** | HR Director | Agent design, hiring, team management | sonnet | Active |
| **Rachel** | Senior Researcher | Deep research, expert profiling, due diligence | sonnet | Active |
